...
INT13:
  ...
  .START_INT13:	
	...
	CMP AH, 0X02	
	JE .DISK_READ					;READ DISK Sectors CHS
	CMP AH, 0X03
	JE .WRITE_DISK					;WRITE DISK Sectors CHS
	CMP AH, 0X05
	JE .FORMAT_TRACK				;Format disk track
	CMP AH, 0X08
	JE .PARAMETERS					;Get current drive parameters (XT & newer, see note Ø)
	CMP AH, 0X0D
	JE .RESET_DISK_SYSTEM 			;RESET DISK
	CMP AH, 0X15
	...
	;Not implemented
	CMP AH, 0X04
	JE .PLACEHOLDER_RETURN			;Verify disk sectors
	CMP AH, 0X06
	JE .PLACEHOLDER_RETURN			;Format track and set bad sector flag (XT & portable)
	CMP AH, 0X07
	JE .PLACEHOLDER_RETURN			;Format the drive starting at track (XT & portable)
	CMP AH, 0X09					
	JE .PLACEHOLDER_RETURN			;Initialize format to disk table
    CMP AH, 0X0A
	JE .PLACEHOLDER_RETURN			;Read long sector (XT & newer, see note Ø)
    CMP AH, 0X0B
	JE .PLACEHOLDER_RETURN			;Write long sector (XT & newer, see note Ø)	
	CMP AH, 0X0C
	JE .PLACEHOLDER_RETURN			;Seek to cylinder
	CMP AH, 0x0D
	JE .PLACEHOLDER_RETURN			;Alternate disk reset (XT & newer, see note Ø)
    CMP AH, 0x0E
	JE .PLACEHOLDER_RETURN			;Read sector buffer (XT & portable only)
    CMP AH, 0x0F
	JE .PLACEHOLDER_RETURN			;Write sector buffer (XT & portable only)	
	CMP AH, 0X10
    ...
    CMP AH, 0X17
	JE .PLACEHOLDER_RETURN			;Set dasd type for format (XT BIOS from 1/10/86 & newer)
    CMP AH, 0X18
	JE .PLACEHOLDER_RETURN			;Set media type for format (BIOS date specific)
    CMP AH, 0X19
	JE .PLACEHOLDER_RETURN			;Park fixed disk heads (AT & newer)
    CMP AH, 0X1A
	JE .PLACEHOLDER_RETURN			;Format ESDI drive unit (PS/2 50+)
...
BELOW_TABLE_SIZE_MESSAGE:
	DB 0x0D, 0x0A, 'Device smaller than ROM parameter table.',0x0D, 0x0A,0
...
.NOT_BOOTABLE_MBR:
	...
	.NOT_MBR_MESSAGE:
	DB 'NO BOOT SIGNATURE MBR', 0X0D, 0X0A,0
	.END_NOT_MBR_MESSAGE:
...
.NOT_BOOTABLE_VBR:
	...
	.NOT_VBR_MESSAGE:
	DB 'NO BOOT SIGNATURE VBR', 0X0D, 0X0A,0
	.END_NOT_VBR_MESSAGE:
...
.NO_BOOTABLE_PARTITION:
	...
	DB 'NO BOOT PARTITION FOUND', 0X0D, 0X0A,0
...
loadedMsg	DB 'Boot Sector loaded...', 0X0D, 0X0A,0
